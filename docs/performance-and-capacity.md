...
Each bulk PDF supports up to 100 certificates. Larger result sets are divided into numbered batches of 100 by the Certificates page. Keep `PDF_CONCURRENCY=1` unless memory testing proves the server can safely handle more; large PDF jobs should queue rather than run many Chromium/PDF jobs at the same time. Single-certificate jobs receive queue priority over bulk jobs that have not started.
