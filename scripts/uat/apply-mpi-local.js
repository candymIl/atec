...
const migrationName = "2026-07-29-magnetic-particle-inspection-module.sql"
const migrationPath = path.join(projectRoot, "database", migrationName)
...
async function main() {
  if (!isLocalHost(process.env.DB_HOST) || process.env.NODE_ENV === "production") {
    throw new Error("Refusing to apply the MPI UAT migration outside a local development database.")
  }
  ...
  const client = new Client({
    ...
    application_name: "atec-mpi-local-uat"
  })
  ...
  try {
    ...
    await client.query(
      ...
      VALUES ($1, $2, 'Applied for local MPI UAT')
    ...
    console.log("Local MPI migration applied and recorded.")
  } catch (error) {
