...
const timesheetRenderer = read("backend/services/workforceTimesheetRenderer.js")
...
assert.ok(routes.includes('req.user.role === "ADMIN" ? ["EMPLOYEE_SUBMITTED","RETURNED"]'),"Admin must be able to approve a corrected returned timesheet")
...
assert.ok(routes.includes('FROM atec.tblusermanagerassignment assignment'),"Manager access must use the controlled employee assignment table")
assert.ok(routes.includes('assignment.manager_user_id=$${values.length}'),"Manager edits must stay limited to linked employees")
...
assert.ok(routes.includes('assignment.manager_user_id=$2'),"Manager schedule access must stay limited to linked employees")
...
assert.ok(read("database/2026-08-19-multiple-employee-managers.sql").includes("CREATE TABLE IF NOT EXISTS atec.tblusermanagerassignment"),"The multi-Manager assignment table must be migrated")
assert.ok(read("deployment/production-migrations.json").includes("2026-08-19-multiple-employee-managers.sql"),"The multi-Manager migration must be included in production")
assert.ok(server.includes("manager_user_ids"),"User management must return and save multiple linked Managers")
...
assert.ok(routes.includes('router.get("/timesheets/submission-status"'),"Reports must compare all active members with a selected day's timesheets")
assert.ok(routes.includes("WHEN t.timesheetid IS NULL THEN 'MISSING'"),"Daily submission status must expose employees with no timesheet")
assert.ok(frontend.includes("Daily submission status"),"Timesheet reports must clearly expose daily completion coverage")
assert.ok(frontend.includes("active members have submitted their timesheet"),"Daily coverage must show submitted versus expected members")
assert.ok(routes.includes('router.get("/payroll-preview"'),"Payroll export selections must have a server-backed preview")
assert.ok(frontend.includes("View payroll preview"),"Payroll users must be able to review selected accepted time before export")
...
assert.ok(frontend.includes("Submit for employee"),"The Admin approval queue must clearly expose employee proxy submission")
assert.ok(frontend.includes("Review / correct entries"),"Admin must be prompted to review employee time before proxy submission")
assert.ok(timesheetRenderer.includes('text("APPROVAL RECORD"'),"The timesheet PDF must present approvals in a dedicated professional section")
assert.ok(timesheetRenderer.includes("formatDate(data.employee_submitted_at, true)"),"Timesheet approval timestamps must use concise local formatting")
assert.ok(timesheetRenderer.includes('path.join(brandRoot, "footer.jpg")'),"The timesheet PDF must use the controlled branded footer")
assert.ok(timesheetRenderer.includes("const rowHeight = Math.max(34"),"Timesheet detail rows must expand cleanly for wrapped content")
assert.ok(frontend.includes("Approve corrected timesheet"),"Admin must have a clear approval action after correcting a returned timesheet")
