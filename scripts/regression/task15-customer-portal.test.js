...
assertIncludes(main, "Inactive · Activate", "Inactive internal users must expose activation beside their username")
assertIncludes(main, "Active · Deactivate", "Active internal users must expose deactivation beside their username")
assertIncludes(main, "&& manager.is_active &&", "Approving-manager choices must exclude inactive accounts")
assertIncludes(main, "Active Admins and Managers only · max 3", "Approving-manager guidance must explain the active-account rule")
...
assertIncludes(certificatesPage, 'isCustomerUser ? "" : `<span><small>Client</small>', "Certificate results must conditionally omit repeated customer details")
assertIncludes(certificatesPage, "isCustomerUser ? asset : tag", "Customer certificate results must show the asset instead of the internal tag-first identity")
...
assertIncludes(certificatesPage, "certificate-section-card certificate-search-card", "Certificate search must have a distinct work-area card")
assertIncludes(certificatesPage, "certificate-section-card certificate-bulk-card", "Bulk certificate actions must have a distinct work-area card")
...
assertIncludes(style, ".user-name-status-toggle.inactive", "The username-level activation control styling is missing")
assertIncludes(style, ".certificate-section-card", "Certificate work areas must have clear section boundaries")
...
assertIncludes(style, ".certificate-worklist", "Customer certificate results must use the compact worklist layout")
