...
assert.ok(server.includes("selected.assetid=$3"),"Inspection linkage must prefer the Job Card containing the inspected asset")
assert.ok(server.includes("visitMatches.length === 1"),"Inspection linkage must prefer an unambiguous visit match")
assert.ok(server.includes("linkedJobCards.rows.length === 1"),"A single matching Job Card must still link automatically")
assert.ok(!server.includes("More than one active Job Card uses this Accelo Job Number"),"Multiple legitimate Job Cards must not block inspection saving")
