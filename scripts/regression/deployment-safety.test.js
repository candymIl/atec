...
assert(deploy.includes('"package-lock.json"'), "Deployment must clean root package-lock drift")
...
assert(deploy.includes("install_packages_if_needed"), "Deployment must reuse unchanged installed packages")
assert(deploy.includes(".atec-package-lock.sha256"), "Dependency reuse must be tied to the package lock fingerprint")
assert(deploy.includes("sha256sum"), "Deployment must fingerprint package lock files")
assert(deploy.includes("npm ci --prefer-offline --no-audit --no-fund"), "Changed dependencies must use a clean, cache-friendly install")
assert(deploy.includes("dependencies changed or are missing"), "Deployment output must explain why packages are installed")
assert(deploy.includes("packages unchanged; reusing installed packages"), "Deployment output must explain when installation is skipped")
