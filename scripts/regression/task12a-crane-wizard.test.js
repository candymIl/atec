...
assertIncludes(backendServer, "certificate.certificateEligibility = evaluateCertificateEligibility(certificate)", "Load-test certificates must use shared eligibility instead of fallback result fabrication")
assert(!backendServer.includes("certificate.results.push(...criteriaRows.map"), "Certificates must not fabricate criteria result rows when saved results are missing")
