...
def brand(d):
    d.rounded_rectangle((86, 54, 188, 154), radius=20, fill=NAVY)
    d.text((112, 67), "A", font=font(58, True), fill=WHITE)
    d.text((215, 67), "ATEC", font=font(46, True), fill=NAVY)
    d.text((218, 119), "INSPECTION PLATFORM", font=font(18, True), fill=MUTED)
...
# 2 — problem
im, d = canvas(); brand(d); title(d, "The challenge", "Compliance should not depend on paperwork")
...
# 3 — identity
im, d = canvas(); brand(d); title(d, "Digital asset identity", "One label connects the physical asset to its records")
if LABEL.exists():
...
# 4 — tap
im, d = canvas(); brand(d); title(d, "Tap to know", "Instant asset information", "Status and certificates—available at the equipment")
...
im, d = canvas(); brand(d); title(d, "Inspect at the asset", "Find it by NFC or QR", "Then follow a controlled inspection or load-test workflow")
...
# 6 — workflow
im, d = canvas(); brand(d); title(d, "Guided and consistent", "Complete inspections. Complete evidence.")
...
# 7 — cert
im, d = canvas(); brand(d); title(d, "From inspection to certificate", "Inspection evidence stays connected")
...
# 8 — management
im, d = canvas(); brand(d); title(d, "Know what needs attention", "Live visibility across every asset")
