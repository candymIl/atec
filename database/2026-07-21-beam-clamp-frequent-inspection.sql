...
WHERE criteria.equiptypeid = equipment_type.equiptypeid
  AND LOWER(TRIM(equipment_type.description)) IN ('beam clamp', 'beam clamps')
...
BEGIN
  IF EXISTS (
    ...
    WHERE LOWER(TRIM(equipment_type.description)) IN ('beam clamp', 'beam clamps')
      AND COALESCE(criteria.inspectioncategory, 'VISUAL') = 'VISUAL'
      AND COALESCE(criteria.inspection_category, '') <> 'FREQUENT_INSPECTION'
  ) THEN
    RAISE EXCEPTION 'Beam clamp visual criteria still contain a non-frequent inspection category';
  END IF;
END $$;
