...
WHERE criteria.equiptypeid = equipment_type.equiptypeid
  AND LOWER(TRIM(equipment_type.description)) = 'crawls (plain/geared)'
...
BEGIN
  IF EXISTS (
    ...
    WHERE LOWER(TRIM(equipment_type.description)) = 'crawls (plain/geared)'
      AND COALESCE(criteria.inspectioncategory, 'VISUAL') = 'VISUAL'
      AND COALESCE(criteria.inspection_category, '') <> 'FREQUENT_INSPECTION'
  ) THEN
    RAISE EXCEPTION 'Crawls (Plain/Geared) visual criteria still contain a non-frequent inspection category';
  END IF;
END $$;
