...
export async function renderTimesheetApprovals() {
  const page = document.querySelector('#page')
  page.innerHTML = `<div class="page-heading"><div><h2>Timesheet Approvals</h2><p>Review exceptions, then approve or return them with a reason.</p></div></div><section id="approvalList" class="filter-card">Loading...</section>`
  try {
...
export async function workforceAction(id, action) {
  const reason = action === 'RETURN' ? window.prompt('Reason for returning this timesheet:') : ''
