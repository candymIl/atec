...
export async function renderMyDay() {
  ...
  page.innerHTML = `<div class="workforce-my-day"><div class="page-heading"><div><h2>My Day</h2><p>Confirm the time copied from job cards and add only exceptions or non-job activities.</p></div></div>
    <section class="filter-card workforce-date-card"><label>Date <input id="workforceDate" type="date" value="${safeAttr(date)}" onchange="showMyDay()"></label></section>
    <section id="workforceDay" class="filter-card workforce-day-card">Loading your day...</section></div>`
  try {
...
export async function viewPayrollPreview() {
  ...
  try {
    ...
    box.innerHTML = `<div class="section-heading"><div><h4>Payroll preview</h4><p class="muted-text">${escapeHtml(selection.from)} to ${escapeHtml(selection.to)} · ${rows.length} accepted timesheet(s)</p></div><button type="button" onclick="document.querySelector('#payrollPreview').hidden=true">Close preview</button></div>
      ...
      <div class="table-scroll"><table><thead><tr><th>Date</th><th>Employee</th><th>Employee no.</th><th>Accelo job numbers</th><th>Job card numbers</th><th>Status</th><th>Normal</th><th>Overtime</th><th>Travel</th><th>Standby</th><th>View</th></tr></thead><tbody>${rows.map(row => `<tr><td>${escapeHtml(row.timesheet_date)}</td><td>${escapeHtml(row.employee_name)}</td><td>${escapeHtml(row.employee_number || '-')}</td><td>${escapeHtml(row.job_numbers || '-')}</td><td>${escapeHtml(row.job_card_numbers || '-')}</td><td>${escapeHtml(row.status.replaceAll('_',' '))}</td><td>${hours(row.normal_hours)}</td><td>${hours(row.overtime_hours)}</td><td>${hours(row.travel_hours)}</td><td>${hours(row.standby_hours)}</td><td><button onclick="window.open('${API_BASE}/workforce/timesheets/${row.timesheetid}/pdf','_blank','noopener')">View PDF</button></td></tr>`).join('') || '<tr><td colspan="11">No HR-accepted timesheets match this payroll selection.</td></tr>'}</tbody></table></div>`
...
export function filterTimesheetHistoryResults() {
  ...
  const rows = historyRows.filter(row => [row.employee_name,row.employee_number,row.job_numbers,row.job_card_numbers,row.status,row.manager_name]
...
export function exportTimesheetHistoryCsv() {
  ...
  const values = [
    ['Date','Employee','Employee Number','Accelo Job Numbers','Job Card Numbers','Status','Normal','Overtime','Travel','Standby','Manager'],
    ...historyRows.map(row => [String(row.timesheet_date).slice(0,10),row.employee_name,row.employee_number,row.job_numbers,row.job_card_numbers,row.status,row.final_normal_hours,row.final_overtime_hours,row.final_travel_hours,row.final_standby_hours,row.manager_name])
  ]
