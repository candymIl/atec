...
window.showJobCards = async function () {
  ...
  if (['ADMIN', 'MANAGER'].includes(currentUser.role)) {
    ...
    button.textContent = 'Follow-ups & Management Reports'
    button.onclick = () => showJobCardFollowups({ apiBase: API_BASE, page, onBack: window.showJobCards, onOpenJob: window.openJobCard })
    page.querySelector('.page-heading').append(button)
  }
  ...
  document.querySelector('#page').innerHTML = `
    ...
    <form id="jobCardForm" class="job-card-form" onsubmit="return false">
      ...
      <section class="filter-card"><h3>Fault, Findings and Work</h3><div class="job-card-text-grid">
        ...
        <label><span><input type="checkbox" id="jcQuoteRequired" ${card.quote_required ? 'checked' : ''}> Quote required — hand recommendation to Accelo</span></label>
        <label><span><input type="checkbox" id="jcReturnRequired" ${card.return_visit_required ? 'checked' : ''}> Return visit required</span></label>
        ${jobCardTextarea('jcReturnReason','Unfinished work / reason for return visit',card.return_visit_reason)}
        <p class="muted-text">These selections create office follow-ups when saved. Existing follow-ups remain in the review list until explicitly closed, even if a selection is later cleared. Out-of-service or restricted status creates an attention item for each asset on this job card.</p>
      </div></section>
